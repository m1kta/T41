/*==============================================================================
  ###IMPORTANT
  - This is a copy of the theme.js file with custom coding. We needed to make
    changes to existing functions.
  - All changes are prefaced with `#EDIT`. Search for this term to see the 
    changes
  - This file is minified as theme-custom.min.js for production


  CHANGES
  - Add Quick View functionality for our new Cross Sells area in ProductSection
    (#CROSS SELLS)
  - #CROSS SELLS and #ORDER LIMITS functionality works differently on
    ProductSection than other sections.
  - Added maximum order limits functionality (#ORDER LIMITS)
  - Add BACK IN STOCK app integration (#BACK IN STOCK)
  - Add Quick View functionality on BlogPostsSection (#BLOG POSTS)
  - Removed autocomplete search functionality (#SEARCH)
  - Amended #TEXT WITH ICONS slider
  - Amend #FILTERS to show only available tags

  EDITED MAY 2020 - by HS
  - Added IIFE for #ORDER LIMITS VALIDATION

  EDITED NOV 2020 by HS
  - VAT implementation which amends ProductVariants and ConvertAll functions

  EDITED DEC 2020 by Andrey (#IS+)
  - VAT is applied to the search results page and search suggestions
  - Currency conversion on search results page and search suggestions

  EDITED FEB 2021 by HS
  - Change VAT implementation so non-GBP currencies divide by 1.2

  EDITED DEC 2021 by HS
  - Change VAT implementation to look for window.Shopify.country variable
  - This ensures that users who have selected a non-GB shipping destination
    do not see the "incl VAT" message
==============================================================================*/
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var Currency = function () {
    ...
    _createClass(Currency, null, [{
      ...
      key: 'convertAll',
      value: function convertAll(selector, variantChange) {
        var _this = this;
         ...
         * #EDIT Nov 2020 by HS
         * #VAT
         * Add hasVAT flag variable which will determine whether extra
        ...
        (selector || document).querySelectorAll('[data-money-convertible]').forEach(function (item) {
          ...
          if (item.getAttribute('data-currency') === newCurrency) {
            if ( hasVAT ) {
              /**
               * #EDIT Dec 2020 by Andrey
               * #VAT #IS+
               * If the amount has already been converted in Search Results,
               * and add the currency sign in front of it.
               */
              let newFormat = moneyFormats[newCurrency][window.theme.currencyConversionMoneyFormat],
                  baseAmount = item.getAttribute('data-base-amount'),
                  cents = parseFloat(baseAmount.replace(/^[^0-9]+|[^0-9.]/g, '', ''), 10) * 100;
              
              if ( newCurrency != 'GBP' ) {
                cents = window.Currency.convert(parseFloat(baseAmount.replace(/^[^0-9]+|[^0-9.]/g, '', ''), 10) * 100, baseCurrency, newCurrency);
              }
              ...
              if (window.theme.currencyConversionRoundAmounts && newCurrency != 'GBP') {
            ...
            else { // set the amount in price field based on the data attributes
              let newFormat = moneyFormats[newCurrency][window.theme.currencyConversionMoneyFormat],
                  baseAmount = item.getAttribute('data-base-amount'),
                  cents = parseFloat(baseAmount.replace(/^[^0-9]+|[^0-9.]/g, '', ''), 10) * 100;
              ...
              // If has no VAT, we divide by 1.2
              cents = cents / 1.2;
          ...
          // Get the base amount which is stored in the HTML
          var baseAmount = item.getAttribute('data-base-amount');

          /**
           * #EDIT
           * #VAT
           * If currency is saved and this is not a variant change, we can
           * just fetch the price from a data attribute.
           *
           * If not, we run complex code.
           */
          if (item.hasAttribute('data-currency-' + newCurrency) && !variantChange) {
             ...
             * #EDIT
             * #VAT
             * Add extra condition to not round amounts if using GBP. We want
             * to ensure that GBP prices are accurate when users get to check
             * out.
             */
            if (window.theme.currencyConversionRoundAmounts && newCurrency != 'GBP') {
              cents = Math.round(cents / 100) * 100;
             ...
             * #EDIT Dec 2020 by Andrey
             * #VAT #IS+
             * Make $ sign appear in front of the converted amount if it's USD
             */
            if ( item.classList.contains('isp_product_price') && newCurrency === "usd" ) {
              item.setAttribute('data-currency-' + newCurrency, '$' + newFormattedAmount);
            }
          }
   ...
   * #EDIT
   * #VAT
   * To use to use the module in isplus-callback.js
   */
...
/***/function (module, __webpack_exports__, __webpack_require__) {
   ...
   * #EDIT OCT 2019 by HS
   * #CROSS SELLS
   * - We need to register the ProductSection so our Quick Views work within
   *   the ProductSection!
   */
    ...
    _createClass(ProductSection, [{
      ...
      value: function _attachListeners() {
       ...
       /**
         * #EDIT OCT 2019 - by HS
         * #CROSS SELLS
         * Add Quick View for Cross Sells on Product Template
         */
     ...
     /**
      ...
      value: function _openQuickView(event, target) {
        ...
        if (__WEBPACK_IMPORTED_MODULE_4__helper_Responsive__["default"].matchesBreakpoint('phone') || __WEBPACK_IMPORTED_MODULE_4__helper_Responsive__["default"].matchesBreakpoint('tablet')) {
        ...
        fetch(productUrl.href, {
          ...
          response.text().then(function (content) {
            ...
            if (window.theme.currencyConversionEnabled) {
              CurrencyHelper.convertAll(modal);
            }
          });
        });
      }
  ...
  var Cart = function () {
    ...
    _createClass(Cart, [{
      ...
      value: function _updateQuantity(event, target) {
         ...
         * #EDIT OCT 2019 by HS
         * #ORDER LIMITS
         * A product or variant can have maximum order limits set in metafields.
         * We store these metafield values in data-attributes which allow us to
         * easily hook into these in our JS.
         *
         * This functionality will check if a variant or product has a maximum
         * order limit and will respond appropriately when a user attempts to
         * update quantities
         */

        /**
         * Store variables and flags as necessary
         */
        var currentQuantity = parseInt(target.getAttribute('data-current-value'));
        var totalQty = false;
        var hasVariantLimit = false;
        var variantLimit = false;
        var variantId = false;

        
        /**
         * Check if we have a maximum order limit and set variable flags.
         */
        if (target.hasAttribute('data-variant-mol')) {
          hasVariantLimit = true;
          variantId = target.getAttribute('data-variant-id');
          variantLimit = parseInt(target.getAttribute('data-variant-mol'));
        } else {
          hasVariantLimit = false;
          variantId = false;
          variantLimit = false;
        };
        ...
        /**
         * 1. If increasing quantity with a maximum order limit we show an alert
         * message and prevent the user from purchasing.
         * 2. We set the totalQty of the line item.
         */
        if (hasVariantLimit && target.getAttribute('data-action') === 'increase-quantity') {
          // If currentQuantity is the limit, prevent buying
          if (currentQuantity === variantLimit) { /* [1] */
            alert('Sorry, you can only order a maximum of ' + variantLimit);
            event.preventDefault();
            return;
          }

          totalQty = parsedQuantity + currentQuantity; /* [2] */
        };


        /**
         * Set the parsedQuantity depending on whether we're using the input element
         */
        ...
        /**
         * 1. If user increases quantity on input above variant limit, we show
         *    a message.
         * 2. We reset the input value to what it was before
         * 3. We set totalQty of lineitem
         */
        if (hasVariantLimit && target.tagName === 'INPUT') { /* [1] */
          if (parsedQuantity > variantLimit) {
            alert('Sorry, you can only order a maximum of ' + variantLimit);
            target.value = parseInt(target.getAttribute('data-current-value')); /* [2] */
            return;
          }

          totalQty = parsedQuantity + currentQuantity; /* [3] */
        };
        ...
        fetch(window.theme.localeRootUrl + '/cart/change.js', {
           ...
           * #EDIT OCT 2019 by HS
           * #ORDER LIMITS
           * We are storing variant limits and quantities in localStorage. This
           * is the only way to implement the maximum limits using AJAX across
           * the site.
           *
           * 1. If a user removes an item, we remove items from localStorage
           * 2. If a user successfully updates an item, we update localStorage
           */
          if (hasVariantLimit) {
            var id = target.getAttribute('data-line-id');
            if (parsedQuantity === 0) { /* [1] */
              localStorage.removeItem('variant_' + variantId + '_qtyInCart');
              localStorage.removeItem('variant_' + variantId + '_MOL');
            } else { /* [2] */
              localStorage.setItem('variant_' + variantId + '_qtyInCart', parsedQuantity);
              localStorage.setItem('variant_' + variantId + '_MOL', variantLimit);
            }
          }
          ...
          if (window.theme.template !== 'cart') {
            content.text().then(function (html) {
              ...
              if (window.theme.currencyConversionEnabled) {
                  ...
                  if (currentCurrency != 'GBP' || window.Shopify.country != 'GB') {
                    _this12.element.querySelector('.vat-message').style.display = 'none';
                  }
            ...
            content.text().then(function (html) {
              ...
              if (window.theme.currencyConversionEnabled) {
                __WEBPACK_IMPORTED_MODULE_1__helper_Currency__["default"].convertAll(originalCart);
              }
            });
          }
        });
  ...
  var ProductVariants = function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _onVariantChanged(previousVariant, newVariant) {
        ...
        if (window.theme.currencyConversionEnabled) {
          __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].convertAll(this.element, true);
        }
      ...
      value: function _updateProductPrices(newVariant, previousVariant) {
         ...
         * #EDIT Nov 2020 - by HS 
         * #VAT 
         * Product price liquid has been amended to include a `data-base-amount`
         * which is needed to help with VAT calculations. This is now reflected in the
         * JS below.
         */
        ...
        if (!newVariant) {
          ...
          if (newVariant['compare_at_price'] > newVariant['price']) {
            var newPrice = __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].formatMoney(newVariant['price'], window.theme.moneyFormat);
            var newCompareAtPrice = __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].formatMoney(newVariant['compare_at_price'], window.theme.moneyFormat);

            productPrices.innerHTML += '<span class="price price--highlight" data-money-convertible data-base-amount="' + newPrice + '">' + newPrice + '</span>';
            productPrices.innerHTML += '<span class="price price--compare" data-money-convertible data-base-amount="' + newCompareAtPrice + '">' + newCompareAtPrice + '</span>';
          } else {
            var newPrice = __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].formatMoney(newVariant['price'], window.theme.moneyFormat);
            productPrices.innerHTML += '<span class="price" data-money-convertible data-base-amount="' + newPrice + '">' + newPrice + '</span>';
          }
      ...
      value: function _updateDiscountLabel(newVariant, previousVariant) {
        ...
        if (!newVariant || !(newVariant['price'] < newVariant['compare_at_price'])) {
          ...
          if (window.theme.discountMode === 'percentage') {
            ...
            var newSavings = __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].formatMoney(newVariant['compare_at_price'] - newVariant['price'], window.theme.moneyFormat);
            savings = '<span data-money-convertible data-base-amount="' + newSavings + '">' + newSavings + '</span>';
          }
      ...
      value: function _updateUnitPrice(newVariant, previousVariant) {
        ...
        if (!newVariant || !newVariant['unit_price_measurement']) {
          ...
          var newUnitPrice = __WEBPACK_IMPORTED_MODULE_0__helper_Currency__["default"].formatMoney(newVariant['unit_price'], window.theme.moneyFormat);

          unitPriceMeasurement.parentNode.style.display = 'block';
          unitPriceMeasurement.querySelector('.unit-price-measurement__price').setAttribute('data-base-amount', newUnitPrice);
          unitPriceMeasurement.querySelector('.unit-price-measurement__price').innerHTML = newUnitPrice;
      ...
      * #EDIT OCT 2019 by HS
      * #BACK IN STOCK
      * - Integrate Back in Stock app so it works with variants.
      * - The notify button is an element with an ID targeted by the app JS.
      *   It's hidden using an inline display style.
      * - The notify button does not work on quick view.
      */
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        /**
         * 1. Store notify button element as a variable
         */
        var addToCartButtonElement = this.element.querySelector('.product-form__add-button'),
            ...
            notifyButtonElement = this.element.querySelector('.product-form__notify-button'); /* [1] */
        ...
        if (!newVariant) {
          ...
          /**
           * 2. If variant is available we
           *    hide the notify button, and show the add to cart button
           */
          if (newVariant['available']) { /* [2] */
            notifyButtonElement.style.display = 'none';
            ...
            /**
             * 3. If variant is NOT available and we are NOT on the quick view, 
             *    we hide the add to cart button, and show the notify button. We *    also set the data-variant-id so the current variant is 
             *    selected in the popup modal.
             */

            addToCartButtonElement.style.display = 'none';
            notifyButtonElement.setAttribute('data-variant-id', newVariant.id);
            notifyButtonElement.style.display = 'block';
            addToCartButtonElement.setAttribute('disabled', 'disabled');
            addToCartButtonElement.classList.add('button--disabled');
            addToCartButtonElement.classList.remove('button--primary');
            addToCartButtonElement.removeAttribute('data-action');
      ...
      value: function _addToCart(event, target) {
        ...
        var hasNoVariants = false;

        /**
         * #EDIT OCT 2019 by HS
         * #CROSS SELL & ORDER LIMITS
         * When using the Cross Sell with Order Limits, we must be aware that
         * our cross sells are within an existing product section. Some cross
         * sell products have no variants and show an Add to Cart button. By
         * default when we click on ATC, the main product we're on will be
         * added, not the cross sell product! So, we have a JS hook for 
         * single products w/ no variants. We check if the hook exists and set
         * a variable flag.
         */
        if (target.hasAttribute('data-no-variants')) {
          hasNoVariants = true;
        }
         ...
         */
        if (hasNoVariants) {
          selectedVariant = formElement.querySelector('.hidden-input__single-variant');
          currentVariant = selectedVariant.getAttribute('data-variant-id');
        } else if (this.masterSelector) {
          selectedVariant = this.masterSelector.options[this.masterSelector.selectedIndex];
        ...
        /**
         * If the selected variant has an order limit
         */
        if (selectedVariant && selectedVariant.hasAttribute('data-variant-mol')) {

          // 1. Store the maximum order limit value
          variantLimit = parseInt(selectedVariant.getAttribute('data-variant-mol'));

          // 2. Check localStorage to see if item is in cart and the quantity
          qtyInCart = parseInt(localStorage.getItem('variant_' + currentVariant + '_qtyInCart'));

          // 3. Get the quantity that a user wants to add
          var qtyToAdd = parseInt(formElement.querySelector('[name="quantity"]').value);

          // 4. Get the totalQty depending on if we have qty already in cart or not
          if (qtyInCart) {
            totalQty = qtyInCart + qtyToAdd;
          } else {
            totalQty = qtyToAdd;
          }

          // 5. Set allowPurchase flag depending on if total qty is over or under
          //    maximum order limit
          if (totalQty > variantLimit) {
            allowPurchase = false;
          } else {
            allowPurchase = true;
          }
        ...
        /**
         * If we have a limit and are over the maximum order limit:
         * 1. Show an alert message
         * 2. Reset the page by removing disabled attribute, and theme cart loading event
         * 3. Return
         */
        if (variantLimit && !allowPurchase) {
          alert('Sorry, you can only order a maximum of ' + variantLimit);
        ...
        fetch(window.theme.localeRootUrl + '/cart/add.js', {
          ...
          if (response.ok) {
             ...
             * #ORDER LIMITS
             *
             * On successful add to cart, we check if we have a maximum order limit
             * and store the line item quantity and maximum order limit in localStorage
             */
            if (variantLimit) {
              localStorage.setItem('variant_' + currentVariant + '_qtyInCart', totalQty);
              localStorage.setItem('variant_' + currentVariant + '_MOL', variantLimit);
            }
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var SearchBar = function () {
    ...
    _createClass(SearchBar, [{
       ...
       * #SEARCH
       * 
       * The theme has an autocomplete search. We don't need this as we are using
       * the InstantSearch+ app. Therefore, we comment out a range of events
       * below.
       */
      key: '_attachListeners',
      value: function _attachListeners() {
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  /**
   * #EDIT OCT 2019 by HS
   * #BLOG POST FEATURED PRODUCTS
   * Amended this module to allow for add to cart and quick view on featured products
   */
  /* harmony import */var __WEBPACK_IMPORTED_MODULE_0__helper_Form__ = __webpack_require__(2);
  /* harmony import */var __WEBPACK_IMPORTED_MODULE_1__helper_Responsive__ = __webpack_require__(0);
  /* harmony import */var __WEBPACK_IMPORTED_MODULE_3__ProductSection__ = __webpack_require__(4);

  var BlogPostSection = function () {
    function BlogPostSection(element) {
      ...
      value: function _reload(pushState) {
        ...
        return fetch(sectionUrl, {
          ...
          response.text().then(function (content) {
             ...
             * #FILTERS
             * 
             * We change the filters behaviour to only show available tags.
             * 1. Store variable of new filters parent element
             * 2. Comment out active filters AJAX as we no longer need
             * 3. Use AJAX to reload the filters section
             */
            var filtersSection = _this37.element.querySelector('.js-collection-filters'); /* [1] */
            var filtersDrawer = _this37.element.querySelector('.js-collection-drawer'); /* [2] */
            ...
            */ /* [2] */
            ...
            if (activeFilters) {
              filtersSection.innerHTML = tempElement.querySelector('.js-collection-filters').innerHTML; /* [3] */
              filtersDrawer.innerHTML = tempElement.querySelector('.js-collection-drawer').innerHTML; /* [3] */
            }
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var TextWithIconsSection = function () {
    function TextWithIconsSection(element) {
       ...
       * #TEXT WITH ICONS
       *
       * 1. Set pageDots to false
       * 2. Added `groupCells: 2` attribute to show 2 items at a time on mobile 
       */
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  (function () {
    ...
    (function () {
      ...
      var vatMessage = document.querySelector('.vat-message');
      ...
      if (window.theme.currencyConversionEnabled) {
        ...
        if (newCurrency == 'GBP' && vatMessage && window.Shopify.country == 'GB') {
          document.querySelectorAll('.vat-message').forEach(function (item) {
      ...
      if (currencySelectors.length > 0) {
          ...
          /**
           * #EDIT
           * On initial page load currency is not set if using default GBP.
           * We set it so we have a hook for other functionality
           */
          ...
          /**
           * #EDIT
           * If GBP we show vat messages
           */
          if (currentCurrency == 'GBP' && vatMessage && window.Shopify.country == 'GB') {
            document.querySelectorAll('.vat-message').forEach(function (item) {
              item.style.display = 'inline';
            });
          } else if (vatMessage) {
     ...
     * 
     * Some products have maximum order limits. We are validating these limits
     * by storing values in localStorage which will prohibit a customer from
     * purchasing more. However, cart items are only stored for 14 days and/or
     * will be removed if an item is out of stock. Which means in some cases,
     * a user may have no item in the cart but still have the localStorage value
     * limiting their purchase.
     *
     * Therefore we need a method to compare localStorage to the Cart.
     *
     * Ideally, we would look to only run this once per session but it may be
     * safer to run all the time for now.
     */
    (function () {
      /**
       * #EDIT Dec 2020 by Andrey
       * #VAT #IS+
       * Load the first module to use price conversion in Search Results and Suggestions
       */      
      window.Conversion = require(1);

      /**
       * Store our localStorage values in an array
       */
      var localStorageArr = [];

      /**
       * Initial function which will check localStorage for items, and update
       * our localStorageArr value.
       */
      checkStorageForItems();

      /**
       * If we have localStorageItems we run checkCartForItems(), otherwise we
       * return.
       */
      ...
      /**
       * Check localStorage for items function. We loop through localStorage,
       * and push any matching items to the localStorageArr
       */
      function checkStorageForItems() {
        for (var i = 0; i < localStorage.length; i++) {
          if (localStorage.key(i).startsWith('variant_')) {
      ...
      /**
       * Check the cart for items. We use fetch and AJAX api to get the cart
       * JSON.
       */
      function checkCartForItems() {
        fetch(window.theme.localeRootUrl + '/cart.json', {
          credentials: 'same-origin',
          method: 'GET'
        }).then(function (response) {
          ...
          /**
           * If we have items in the cart, we loop through our localStorage
           * items, strip them to their variant IDs and then check if these
           * IDs exist in the cart Items
           */
          if (data.item_count > 0) {
            for (var i = 0; i < localStorageArr.length; i++) {
              var lsValue = localStorageArr[i];
              var lsValueStripped = lsValue.replace('variant_', '');
              lsValueStripped = lsValueStripped.replace('_qtyInCart', '');
              lsValueStripped = lsValueStripped.replace('_MOL', '');
              checkIfCartItemExists(lsValueStripped);
            }
          } else {
            // If we do not have cart items, we delete all items from localStorage
            deleteItemsFromStorage();
          }


          /**
           * Loop through the cart items and check if a variant ID matches the
           * localStorage item we have. If there is a match, we return.
           * If there is no match, we delete the item from localStorage.
           */
          function checkIfCartItemExists(lsValueStripped) {
            for (var i = 0; i < data.items.length; i++) {
              if (data.items[i].variant_id == lsValueStripped) {
              } else {
                deleteItemFromStorage(lsValueStripped);
              }
            }
          }
          ...
          /**
           * Function to delete a single item from localStorage if it is not
           * in the cart.
           */
          function deleteItemFromStorage(lsValueStripped) {
            var lsValueMOL = 'variant_' + lsValueStripped + '_MOL';
            var lsValueQty = 'variant_' + lsValueStripped + '_qty';

            localStorage.removeItem(lsValueMOL);
            localStorage.removeItem(lsValueQty);
          }
        });
      }

      /**
       * A broad function that will delete items from localStorage if we have
       * no cart items.
       */
      function deleteItemsFromStorage() {
        for (var i = 0; i < localStorageArr.length; i++) {
          var value = localStorageArr[i];
